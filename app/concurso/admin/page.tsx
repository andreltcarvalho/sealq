...
import { Button } from "@/components/concurso/ui/Button";
import { StatCard } from "@/components/concurso/ui/StatCard";
import { FilterBar } from "@/components/concurso/ui/FilterBar";
import { Pagination } from "@/components/concurso/ui/Pagination";
import { Table } from "@/components/concurso/ui/Table";
...
export default function ConcursoAdminPage() {
  ...
  const handleLogout = useCallback(() => router.push("/concurso/login"), [router]);
  const handleGoToEvaluators = useCallback(
    () => router.push("/concurso/admin/avaliadores"),
    [router],
  );

  // 1. FUNÇÃO MEMORIZADA PARA NAVEGAR ATÉ A TELA DE CRITÉRIOS
  const handleGoToCriteria = useCallback(
    () => router.push("/concurso/admin/criterios"),
    [router],
  );

  const handleAction = useCallback(
    (id: string) => router.push(`/concurso/admin/avaliacoes/${id}`), // <-- Altere de /avaliar/ para /evaluation/
    [router],
  );
